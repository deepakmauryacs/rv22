    ...
    <div class="card-body">
        ...
        <form>
            ...
            <div class="table-responsive">
                ...
                <table class="product-listing-table table-indent w-100" id="indent-table">
                    <thead>
                        <tr>
...
['data' => 'dept_code'],
['data' => 'cost_code'],
['data' => 'product_block', 'orderable' => false, 'searchable' => false],
['data' => 'product_specs'],
['data' => 'product_size'],
['data' => 'uom'],
['data' => 'product_brand'],
['data' => 'created_at'],
['data' => 'quantity'],
['data' => 'rfq_quantity_sum', 'orderable' => false, 'searchable' => false],
]
: [
['data' => 'checkbox', 'orderable' => false, 'searchable' => false],
['data' => 'indent_no'],
...
<script>
    $(function() {
            ...
            let isTemplate1 = {{ $view_template }};
            var table = $('#indent-table').DataTable({
                processing: true,
                serverSide: true,
                searching: false,
                paging: true,
                ...
                destroy: true,
                ajax: {
                    url: '{{ route('buyer.apiIndent.list') }}',
                    data: (d) => {
                        const isTemplate1 = '{{ $view_template }}';
                        d.branch_id = $('#branch_id').val();
                        d.product_name = $('input[name=product_name]').val();
                        d.product_id = $('input[name=product_id]').val();
                        d.indent_no = $('input[name=indent_no]').val();

                        if (isTemplate1 == 1) {
                            d.division_code = $('input[name=division_code]').val();
                        } else {
                            d.plant = $('input[name=plant]').val();
                        }
                    },
                },
                order: [],
            ...
            // When search button is clicked, reload DataTable
            $('form').on('submit', function(e) {
                e.preventDefault();
                table.ajax.reload();
            });

            // Bind keyup events on filter inputs
            $('input[name=product_name], input[name=product_id], input[name=indent_no], input[name=division_code], input[name=plant]')
            ...
            // Re-bind JS events for dynamic HTML after draw
            $('#indent-table').on('draw.dt', function() {
                // For example, attach click events again if needed
                $('.rfqList').click(function() {
                    let indentId = $(this).data('indent_id');
                    console.log('RFQ clicked for indent', indentId);
                    // open modal or ajax load RFQ details
                });
            });

            // Re-bind after every redraw
            table.on('draw', function() {
                bindProductBlockEvents();
            });
        ...
        function bindProductBlockEvents() {
            // Use delegation for dynamic elements
            $('#indent-table').off('click', '.toggle-trigger').on('click', '.toggle-trigger', function() {
                const block = $(this).closest('.product-block');
                block.find('.form-container').removeClass('d-none');
                block.find('.delete-trigger1').removeClass('d-none');
                $(this).addClass('d-none');
            });

            $('#indent-table').off('click', '.delete-trigger1').on('click', '.delete-trigger1', function() {
                const block = $(this).closest('.product-block');
                block.find('.form-container').addClass('d-none');
                block.find('.toggle-trigger').removeClass('d-none');
                block.find('.update-product-button').addClass('d-none').removeClass('d-inline-block');
                block.find('.product-name').val('');
                block.find('.product-id').val('');
                block.find('.division-category').html('');
            });

            $('#indent-table').off('keyup', '.product-name').on('keyup', '.product-name', function() {
                const input = $(this);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.length > 0) {
                            suggestions.html(
                                `<p class="p-2">Showing result for "<strong>${query}</strong>" – <strong>${data.length}</strong> records found</p>`
                            );
                        ...
                        `);
                            ...
                            suggestions.html(
                                `<p class="p-2">No Product Found For <strong>"${query}"</strong></p>`
                                );
                        }
                    },
                    error: function(xhr) {
            ...
            $('#indent-table').off('click', '.product-item').on('click', '.product-item', function(e) {
                e.preventDefault();
                const item = $(this);
                const block = item.closest('.product-block');
                block.find('.product-name').val(item.data('name'));
                block.find('.product-id').val(item.data('id'));
                block.find('.division-category').html(item.data('division'));
                block.find('.product-suggestions').hide();
                block.find('.update-product-button').removeClass('d-none');
                block.find('.delete-trigger1').removeClass('d-none');
            });

            $(document).off('click.suggestions').on('click.suggestions', function(e) {
                if (!$(e.target).closest('.product-block').length) {
...
<script>
    /***:- view rfq list  -:***/
        ...
        $(document).on('click', '.rfqList', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                url: '{{ route('buyer.apiIndent.getRfqList') }}',
                type: 'POST',
                data: {
                    id: indentId,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(response) {
                    ...
                    if (response.length === 0) {
                        tbody.append(`
                            <tr><td colspan="5" class="text-center">No RFQ Found</td></tr>
                        `);
                    } else {
                        response.forEach(function(item) {
                            tbody.append(`
                                <tr>
                                    ...
                                    <td class="text-center"><input disabled type="text" class="form-control" value="${((item.rfq.buyer_rfq_status==8||item.rfq.buyer_rfq_status==9||item.rfq.buyer_rfq_status==10)?'Yes':'No')}" readonly></td>
                                    <td class="text-center"><input disabled type="text" class="form-control" value="${item.quantity ?? ''}" readonly></td>
                                    <td class="text-center">
                                        <a href="{{ url('buyer/rfq/rfq-details') }}/${item.rfq_id}" target="_blank">View Details</a>
                                    </td>
                                </tr>
                            `);
                        });
                    ...
                    // Show modal after filling table
                    let modal = new bootstrap.Modal(document.getElementById('activeRfqDetailsModal'));
                    modal.show();
                },
                error: function(xhr) {
                    toastr.error('Something went wrong while fetching RFQ list.');
        ...
        $('#rftBtn').on('click', function() {
            ...
            $.ajax({
                url: '{{ route('buyer.apiIndent.getProductList') }}',
                type: 'POST',
                data: {
                    ids: productIds,
                    _token: '{{ csrf_token() }}'
