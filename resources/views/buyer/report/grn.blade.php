...
@section('content')
    <div class="card rounded">
        <div class="card-header bg-white">
            <div class="d-flex align-items-center justify-content-between mb-3">
                <h1 class="font-size-22 mb-0">GRN Report</h1>
        ...
        <div class="card-body">
            ...
            <div class="row g-3 pt-3 mb-3">
                ...
                <div class="col-6 col-sm-auto">
                    <button type="button"
                        class="ra-btn ra-btn-outline-danger w-100 justify-content-center font-size-11" onClick="window.location.href='{{ route('buyer.report.grn') }}'">
                ...
                <table class="product-listing-table w-100 dataTables-example1"  id="report-table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom">GST ({{ session('user_currency.symbol', '₹')}})</th>
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom">Frieght / Other Charges ({{ session('user_currency.symbol', '₹')}})</th>
                            ...
                            <th class="text-center border-bottom-dark text-wrap keep-word align-bottom">Amounts ({{ session('user_currency.symbol', '₹')}})</th>
                        </tr>
                    </thead>
                </table>
            </div>
    ...
    @push('exJs')
        @once
            ...
            <script>
                var grnreportlistdataurl        =    "{{ route('buyer.report.grnReportlistdata') }}";
                var exportreportlistGrnurl      =    "{{ route('buyer.report.exportGrnReport') }}";
                var fetchGrnRowdataurl          =    "{{ route('buyer.report.fetchGrnRowdata') }}";
                var editGrnRowdataurl           =    "{{ route('buyer.report.editGrnRowdata') }}";
                var downloadGrnRowdataurl       = "{{ route('buyer.report.downloadGrnRowdata', ['id' => ':id']) }}";
                var deleteExcelUrl="{{route('buyer.delete.export.file')}}";
            </script>
            <script src="{{ asset('public/js/xlsx.full.min.js') }}"></script>
            <script src="{{ asset('public/js/grn.js') }}"></script>
        @endonce
    @endpush
@endsection
