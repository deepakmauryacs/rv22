@extends('buyer.layouts.app', ['title' => 'Forward Auction - Edit Products'])
        ...
        <div class="card rounded">
            <div class="card-header border-0 bg-transparent">
                <div class="d-flex justify-content-between align-items-center">
                    <h1 class="font-size-18">Forward Auction - Edit Products</h1>
                </div>
            </div>
            
            <div class="card-body">
                <form method="post" action="{{ route('buyer.forward-auction.update', $auction->id) }}" enctype="multipart/form-data" id="auction_form">
                    @csrf
                    @method('PUT')
                    <div class="row gx-3">
                        <div class="col-12 col-sm-4 col-md-4 col-lg-auto mb-4 mb-sm-3">
                            <div class="input-group">
                                ...
                                <div class="form-floating">
                                    <input type="text" name="schedule_date" id="schedule_date" class="form-control" placeholder="Select Date" autocomplete="off" value="{{ old('schedule_date', $auction->schedule_date ? \Carbon\Carbon::parse($auction->schedule_date)->format('d/m/Y') : '') }}">
                                ...
                                <div class="form-floating">
                                    <select name="schedule_start_time" id="schedule_start_time" class="form-control SumoUnder">
                                        ...
                                        @for ($hour = 0; $hour < 24; $hour++)
                                            ...
                                            @endphp
                                            <option value="{{ $value }}" {{ old('schedule_start_time', $auction->schedule_start_time) == $value ? 'selected' : '' }}>{{ $display }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                                    ...
                                    <select class="globle-field-changes form-select" id="buyer_branch" name="buyer_branch" aria-label="Floating label select example">
                                        ...
                                        @foreach($branches as $val)
                                            @if($val->branch_name != '')
                                                ...
                                                @endphp
                                                <option value="{{ $val->branch_id }}" data-address="{{ htmlspecialchars($fullAddress) }}" {{ old('buyer_branch', $auction->buyer_branch) == $val->branch_id ? 'selected' : '' }}>
                                                    {{ $val->branch_name }}
                                                </option>
                                            @endif
                                        @endforeach
                        ...
                        <div class="col-12 col-sm-4 col-md-4 col-lg-auto mb-4 mb-sm-3">
                            <div class="input-group">
                                ...
                                <div class="form-floating">
                                    <input type="text" name="branch_address" id="branch_address" class="form-control" placeholder="Branch address" readonly value="{{ old('branch_address', $auction->buyerBranch ? trim($auction->buyerBranch->address . ', ' . $auction->buyerBranch->state_name . ', ' . $auction->buyerBranch->country_name) : '') }}">
                        ...
                        <div class="col-12">
                            <div class="selected-vendor-list height-inherit border rounded p-3 {{ $selectedVendors ? '' : 'd-none' }}">
                                <h3 class="font-size-11 mb-2">Selected Vendors</h3>
                                <div class="vendor-chip-container" id="aliasContainer">
                                     @foreach($selectedVendors as $index => $vendor)
                                        <span class="vendor-chip-item vendor-{{ $vendor->vendor_id }}">
                                           {{ $vendor->vendor_name }}
                                            <span role="button" class="ra-btn ra-btn-link bi bi-x-lg p-0 ms-2 width-inherit font-size-11 remove-alias" onclick="removeVendor(this, {{ $vendor->vendor_id }})"></span>
                                        </span>
                                     @endforeach
                                </div>
                            </div>
                        ...
                        <div class="col-12 py-3">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" id="product_table">
                                    ...
                                    <tbody>
                                        @foreach($auction->products as $index => $product)
                                            <tr id="row{{ $index + 1 }}">
                                                <td class="mw-250 align-top">
                                                    <input type="text" name="product_name[]" class="form-control product-name" maxlength="500" autocomplete="off"
                                                        ...
                                                        placeholder="Type to search products..." value="{{ old('product_name.' . $index, $product->product_name) }}">
                                                    <input type="hidden" name="product_id[]" value="{{ old('product_name.' . $index, $product->id) }}">
                                                    <div class="suggestion-dropdown" id="suggestion-dropdown-row{{ $index + 1 }}" style="display: none;"></div>
                                                    <div class="text-danger error product-name-error"></div>
                                                </td>
                                                <td class="align-top">
                                                    <input type="text" name="specs[]" class="form-control specs" maxlength="500" value="{{ old('specs.' . $index, $product->specs) }}">
                                                    <div class="text-danger error specs-error"></div>
                                                </td>
                                                <td class="align-top">
                                                    <input type="number" name="quantity[]" class="form-control quantity" min="0.01" step="0.01" value="{{ old('quantity.' . $index, $product->quantity) }}">
                                                ...
                                                <td class="mw-120 align-top">
                                                    <select class="form-control uom" name="uom[]">
                                                        <option value="">Select</option>
                                                        @foreach($uoms as $uom)
                                                            <option value="{{ $uom->id }}" {{ old('uom.' . $index, $product->uom ) == $uom->id ? 'selected' : '' }}>{{ $uom->uom_name }}</option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <td class="align-top">
                                                    <input type="number" name="start_price[]" class="form-control price" min="0.01" step="0.01"
                                                        onkeydown="return !(event.key === '+' || event.key === '-')" value="{{ old('start_price.' . $index, $product->start_price) }}">
                                                ...
                                                <td class="align-top">
                                                    <input type="number" name="min_bid_increment_amount[]" class="form-control min-bid-amount" min="0.01" step="0.01"
                                                        onkeydown="return !(event.key === '+' || event.key === '-')" value="{{ old('min_bid_increment_amount.' . $index, $product->min_bid_increment_amount) }}">
                                                ...
                                                <td class="mw-250 align-top">
                                                    ...
                                                    @if($product->file_attachment)
                                                        <div class="mt-2">
                                                            <a href="{{ asset('public/uploads/forward_auction_files/' . $product->file_attachment) }}" target="_blank">View Current File</a>
                                                        </div>
                                                    @endif
                                                ...
                                                <td class="align-top">
                                                    <button type="button" name="remove" id="{{ $index + 1 }}" class="ra-btn small-btn ra-btn-outline-danger text-nowrap btn_remove">
                                                        <span class="bi bi-x-circle font-size-12"></span>
                                                        Remove
                                                    </button>
                                                </td>
                                            </tr>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="remarks" class="form-control height-inherit" rows="3"
                                ...
                                oninput="if(this.value.length > 1000) this.value = this.value.slice(0, 1000);">{{ old('remarks', $auction->remarks) }}</textarea>
                        </div>
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="form-floating">
                                            <input type="text" name="price_basis" class="form-control" id="priceBasis" placeholder="Price Basis"
                                                oninput="if(this.value.length > 255) this.value = this.value.slice(0, 255);" value="{{ old('price_basis', $auction->price_basis) }}">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="form-floating">
                                            <input type="text" name="payment_terms" class="form-control" id="paymentTerm" placeholder="Payment Term"
                                                oninput="if(this.value.length > 255) this.value = this.value.slice(0, 255);" value="{{ old('payment_terms', $auction->payment_terms) }}">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div class="form-floating">
                                            <input type="text" name="delivery_period" class="form-control" id="deliveryPeriod" placeholder="Delivery Period (In Days)" maxlength="3"
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 3);" value="{{ old('delivery_period', $auction->delivery_period) }}">
                                            ...
                                            <select name="currency" id="currency" class="form-select">
                                                @foreach($currencies as $val)
                                                    @if(!empty($val->currency_name))
                                                        ...
                                                        @endphp
                                                        <option value="{{ $currency_val }}" data-symbol="{{ $currency_symbol }}" {{ old('currency', $auction->currency) == $currency_val ? 'selected' : '' }}>
...
$(document).ready(function() {
    ...
    // Initialize vendor_array with existing vendors
    vendor_array = @json($selectedVendors->pluck('vendor_id')->toArray() ?? []);
    ...
    $(document).on('click', '.btn_remove', function() {
        ...
        if ($('#product_table tbody tr').length === 1) {
            alert("At least one product should remain.");
            return false;
        }
        ...
        if ($('#product_table tbody tr').length === 0) {
            ...
            $('#product_table tbody').append(
                ...
                '<div class="text-danger error file-error"></div>' +
                '</td>' +
                '<td class="align-top"><button type="button" name="remove" id="1" class="ra-btn small-btn ra-btn-outline-danger text-nowrap btn_remove"><span class="bi bi-x-circle font-size-12"></span> Remove</button></td>' +
                '</tr>'
            );
        }
    });
        ...
        $('#product_table tbody tr').each(function() {
            ...
            if (productName) {
                if (productNames.includes(productName)) {
                    ...
                    $(this).find('.product-name-error').text('');
                }
            }
        });
        ...
        if (isValid) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('button[type="submit"]').prop('disabled', false).html('<span class="bi bi-check2-square"></span> Update Auction');
...
// Update the removeVendor function to this:
function removeVendor(el, user_id) {
    ...
    // Update the visibility of the selected vendors container
    $(".selected-vendor-list").toggleClass('d-none', vendor_array.length === 0);
    
    // Also remove the vendor from any hidden input fields if they exist
    $(`input[name="vendor_id[]"][value="${user_id}"]`).remove();
}
...
// Update the selectVendor function to this:
function selectVendor(_this, user_id, vendor_name) {
    user_id = parseInt(user_id);
    
    if(!$(_this).hasClass("vendor-added") && vendor_array.length >= 10) {
        alert("Maximum of 10 vendors can be added at once.");
        return false;
    }

    // Toggle the vendor in the array
    if(vendor_array.includes(user_id)) {
        vendor_array = vendor_array.filter(id => id !== user_id);
