@extends('vendor.layouts.app_second', ['title' => 'Add Fast Product', 'sub_title' => ''])
        ...
        <section class="manage-product card">
            <div class="card">
                ...
                <div class="card-body add-product-section">
                    <p class="fw-bold">Start typing the name of your product, options will start appearing in the
                        dropdown, and then select one.</p>
                    ...
                    <div class="table-responsive">
                        <table class="table ra-table ra-table-stripped add-fast-product-table" id="productTable">
                            <thead>
                                ...
                                <th scope="col" class="text-center text-uppercase text-nowrap">Product Name <span
                                        class="text-danger">*</span></th>
                                <th scope="col" class="text-center text-uppercase text-nowrap">Upload Picture</th>
                                <th scope="col" class="text-center text-uppercase text-nowrap">Product Description <span
                                        class="text-danger">*</span></th>
                                <th scope="col" class="text-center text-uppercase text-nowrap">Dealer Type <span
                                        class="text-danger">*</span></th>
                                @if (is_national() == 1)
                                ...
                                <th scope="col" class="text-center text-uppercase text-nowrap">HSN Code <span
                                        class="text-danger">*</span></th>
                                <th scope="col" class="text-center text-uppercase text-nowrap" style="width:200PX;">
                                    Status</th>
                            ...
                            <tbody>
                                <tr data-row-index="1">
                                    ...
                                    <td>
                                        ...
                                        <input type="text" name="pro-name" id="product_name_1"
                                            class="form-control bulk-product-field" value="" autocomplete="off"
                                            maxlength="255" placeholder="Search Product..." data-row-index="1" />
                                        <input type="hidden" name="product_id" id="product_id_1" value="">

                                        <!-- Suggestions dropdown -->
                                        <div class="suggestions-container" id="suggestions-container-1"
                                            style="display: none;">
                                            <!-- Suggestions list -->
                                    ...
                                    <td>
                                        <div class="simple-file-upload">
                                            <input type="file" id="uploadFile-1" class="real-file-input"
                                                style="display: none;" accept="image/jpeg,image/jpg,image/png,image/gif"
                                    ...
                                    <td>
                                        <input type="text" name="pro-desc1" id="ps_desc_1" class="form-control" value=""
                                            maxlength="500" />
                                    </td>
                                    <td>
                                        <select class="form-select" name="select-dealer1" id="dealer_type_1">
                                            ...
                                            <option value="{{ $dealerType->id }}">{{ $dealerType->dealer_type }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </td>
                                    ...
                                    <td>
                                        <input type="text" name="hsn-code1" id="ean_code_1" class="form-control"
                                            maxlength="8" value="" />
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <button class="ra-btn ra-btn-link p-0 mh-inherit remove-row"
                                            aria-label="Remove">
...
document.getElementById("addProductBtn").addEventListener("click", function() {
    ...
    newRow.innerHTML = `
            ...
            <td>
                <div class="simple-file-upload">
                    <input type="file" id="uploadFile-${rowCount}" class="real-file-input" style="display: none;" accept="image/jpeg,image/jpg,image/png,image/gif" onchange="validateImage(this)">
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.length > 0) {
                suggestionsContainer.show();
                suggestionsList.append('<li class="result-message">Showing result for <b>"' +
                    ucwords(query) + '"</b> ' + response.length + ' records found</li>');

                // Loop through the response and append each product to the list
                response.forEach(function(item) {
                    suggestionsList.append('<li data-id="' + item.id +
                        '" class="suggestion-item">' + item.label + '</li>');
                });
            } else {
                suggestionsContainer.show();
                suggestionsList.append('<li class="result-message">No results found for <b>"' +
                    ucwords(query) + '"</b></li>');
...
$(document).on('click', '#submitProducts', function() {
    ...
    rows.forEach((row) => {
        ...
        if ((status == '1' || status == '2') &&
            ...
            formData.append('products[' + rowIndex + '][product_id]', $('#product_id_' + rowIndex)
            .val());
            formData.append('products[' + rowIndex + '][product_name]', $('#product_name_' + rowIndex)
                .val());
    ...
    $.ajax({
        ...
        complete: function() {
            ...
            $("#submitProducts").html(
                '<span class="bi bi-save font-size-12"></span><span class="font-size-11">Submit</span>'
                );
        }
    });
});
