...
<div wire:ignore.self class="modal fade" id="composeModal" tabindex="-1" aria-labelledby="composeModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered modal-md ">
        <form
            @if ($draftEditMode) wire:submit.prevent="updateDraftData"
            @else
            wire:submit.prevent="composeMessage" @endif
            class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    <input type="text" wire:model.live="subject" id="subject" class="form-control"
                        placeholder="Subject" @disabled($replyMessageText)>
                    @error('subject')
                ...
                <div class="mb-3" wire:ignore>
                    <textarea wire:model.live="message" id="msg" class="form-control height-inherit" placeholder="Your message..." rows="4"></textarea>
                    @error('message')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
            ...
            <div class="modal-footer">
                <button type="submit" @disabled($errors->isNotEmpty())
                    class="ra-btn ra-btn-primary text-uppercase text-nowrap font-size-11">{{ $draftEditMode ? 'Send' : 'Send' }}</button>
