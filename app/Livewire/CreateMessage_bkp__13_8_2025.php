...
class CreateMessage extends Component
{
    ...
    * TODO: will break it in micro function
    */
    public function msgListing_bkp_6_7_2025()
    {
    ...
    {
        try {
            $this->applyMessageType();
            $inbox = $this->baseInboxQuery();

            // apply listing type specific filters & joins
            [$inbox, $rawQuery] = $this->applyListingType($inbox);

            // search filter
            $inbox = $this->applySearchFilter($inbox);

            // favourites join
            [$inbox, $rawQuery] = $this->applyFavouriteJoin($inbox, $rawQuery);

            // filter by parent id if required
            if ($this->id) {
                $inbox->where('messages.parent_id', $this->id);
            }
            ...
            return $inbox->paginate($this->perPage);
        } catch (\Throwable $th) {
            logger()->error($th);
            throw $th;
        }
    ...
    private function applyMessageType()
    {
        $user = Auth::user();
        $this->type_message = (($user->user_type == 2 && $this->message_type == 'buyer')
            || ($user->user_type == 1 && $this->message_type == 'vendor')) ? 2 : 3;
    }

    private function baseInboxQuery()
    {
        return DB::table('messages')
            ->join('message_statuses', 'messages.id', '=', 'message_statuses.message_id')
            ->join('users as sender_user', 'sender_user.id', '=', 'message_statuses.sender_id')
            ->join('users as receiver_user', 'receiver_user.id', '=', 'message_statuses.receiver_id')
            ->where('messages.type_message', $this->type_message);
    }

    private function applyListingType($inbox)
    {
        $auth_id = Auth::id();
        ...
        messages.subject,
        messages.message,
        messages.email_label_id,
        messages.last_ip,
        messages.status,
        messages.type_message,
        messages.created_at,
        message_statuses.id as message_statuses_id,
        message_statuses.message_id,
        message_statuses.sender_id,
        message_statuses.receiver_id,
        message_statuses.sender_send_status,
        message_statuses.receiver_inbox_status,
        message_statuses.sender_draft_status,
        message_statuses.sender_trash_status,
        message_statuses.receiver_trash_status,
        message_statuses.updated_at,
        ...
        switch ($this->listing_type) {
            case 'inbox':
                ...
                $rawQuery = $this->joinBuyerOrVendor($inbox, $rawQuery, true);
                $inbox = $this->applyLatestMessageJoin($inbox);
                break;
            ...
            case 'send':
                ...
                $rawQuery = $this->joinBuyerOrVendor($inbox, $rawQuery, false);
                $inbox = $this->applyLatestMessageJoin($inbox);
                break;
            ...
            case 'trash':
                $inbox = $this->applyTrashFilter($inbox, $rawQuery, $auth_id);
                $inbox = $this->applyLatestMessageJoin($inbox);
                break;
        }

        return [$inbox, $rawQuery];
    }

    private function joinBuyerOrVendor($inbox, $rawQuery, $isInbox = true)
    {
        if ($this->message_type == 'buyer') {
            $inbox->leftJoin('buyers', 'buyers.user_id', '=', $isInbox ? 'message_statuses.sender_id' : 'message_statuses.receiver_id');
            $inbox->where(($isInbox ? 'sender_user' : 'receiver_user') . '.user_type', 1);
            $rawQuery .= ", buyers.legal_name as buyer_legal_name, buyers.buyer_code as buyer_code";
        } elseif ($this->message_type == 'vendor') {
            $inbox->leftJoin('vendors', 'vendors.user_id', '=', $isInbox ? 'message_statuses.sender_id' : 'message_statuses.receiver_id');
            $inbox->where(($isInbox ? 'sender_user' : 'receiver_user') . '.user_type', 2);
            $rawQuery .= ", vendors.legal_name as vendor_legal_name, vendors.vendor_code as vendor_code";
        }
        return $rawQuery;
    }

    private function applyTrashFilter($inbox, &$rawQuery, $auth_id)
    {
        $inbox->where(function ($query) use ($auth_id) {
            $query->where(function ($q) use ($auth_id) {
                $q->where('message_statuses.sender_id', $auth_id)
                    ->whereBetween('message_statuses.sender_trash_status', [1, 2]);
            })->orWhere(function ($q) use ($auth_id) {
                $q->where('message_statuses.receiver_id', $auth_id)
                    ->whereBetween('message_statuses.receiver_trash_status', [1, 2]);
            });
        });
    ...
    private function applyLatestMessageJoin($inbox)
    {
        $subQuery = clone $inbox;
        $subQuery->select(DB::raw('MAX(messages.id) as id'))->groupBy('messages.parent_id');
        $inbox->joinSub($subQuery, 'latest', function ($join) {
            $join->on('messages.id', '=', 'latest.id');
        });
        return $inbox;
    ...
    private function applySearchFilter($inbox)
    {
        if ($this->search) {
            $inbox->where(function ($query) {
                $query->where('messages.subject', 'like', '%' . $this->search . '%');
                if ($this->listing_type == 'inbox') {
                    $query->orWhere('sender_user.name', 'like', '%' . $this->search . '%');
                } elseif (in_array($this->listing_type, ['send', 'draft'])) {
                    $query->orWhere('receiver_user.name', 'like', '%' . $this->search . '%');
                } elseif ($this->listing_type == 'trash') {
                    $query->orWhere('sender_user.name', 'like', '%' . $this->search . '%')
                        ->orWhere('receiver_user.name', 'like', '%' . $this->search . '%');
    ...
    private function applyFavouriteJoin($inbox, $rawQuery)
    {
        $auth_id = Auth::id();
        $inbox->leftJoin('message_favourites as f1', function ($join) use ($auth_id) {
            $join->on('messages.id', '=', 'f1.message_id')->where('f1.user_id', '=', $auth_id);
        })->leftJoin('message_favourites as f2', function ($join) use ($auth_id) {
            $join->on('messages.parent_id', '=', 'f2.message_id')->where('f2.user_id', '=', $auth_id);
        });

        $rawQuery .= ", CASE WHEN f1.id IS NOT NULL THEN 1 WHEN f2.id IS NOT NULL THEN 1 ELSE 0 END as is_fav";
        return [$inbox, $rawQuery];
    }
    ...
    public function markSelectedAsFavourite(Request $request)
    {
        try {
            ...
            foreach ($this->selectedMessages as $item) {
                ...
                $this->toggleFavourite($messages['parent_id']);
            }
            $allFavourited = true;

            $this->selectAllFavourite = !$allFavourited;
        } catch (\Throwable $th) {
            logger()->error($th);
            throw $th;
        }
