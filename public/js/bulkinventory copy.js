$(document).ready(function () {
    ...
    $("#form").validate({
        submitHandler: function(form, event) {
            ...
            $.ajax({
                ...
                //         showimportdata(response.data, response.uom_list, response.invt_type_list);
                //         $('#import_product').val('');
                //     } else {
    ...
    // function showimportdata(responsedata, uom_list, invt_type_list) {
    //     const data1 = [];
    //     let cnts = 1;

    //     responsedata.forEach((p_data, i) => {
    //         if (p_data.Product_Name) {
    //             let j = i + 1;

    //             // Decide row color and status
    //             const row_color = (!p_data.action || /^(New Product|Product Verified)$/i.test(p_data.action)) ? 'text-success' : 'text-danger';
    //             const status = row_color === 'text-success' ? 1 : 2;

    //             let row = [
    //                 cnts,
    //                 `<span style="vertical-align: bottom;" class="product-message ${row_color}" id="product_message_${j}">${p_data.action || 'Product verified'}</span>`,

    //                 `<span class="remove-rfq-record btn-rfq" data-srno="${p_data.srno}"><i class="bi bi-trash3 text-danger remove-bulk-rfq-btn"></i></span>`,
    ...
    //                                 <div id="suggestions_${j}">
    //                                     <div class="keyword_suggestion">
    //                                     </div>
    //                                 </div>
    //                             </div>
    //                             <ul class="suggestProduct" id="suggestProduct_${j}"></ul>
    ...
    //                 `<input type="text" name="ps_spec_[${j}]" id="ps_spec_${j}" title="${p_data.Product_Specification}" readonly value="${truncate(p_data.Product_Specification, 100)}" class="form-control readonly-text-field desc-details-field" tab-index="${j}">`,
    //                 `<input type="text" name="ps_size_[${j}]" id="ps_size_${j}" title="${p_data.Product_Size}" readonly value="${truncate(p_data.Product_Size, 100)}" class="form-control readonly-text-field desc-details-field" tab-index="${j}">`,
    //                 `<input type="text" name="ps_opening_stock_[${j}]" id="ps_opening_stock_${j}" title="${p_data.Opening_Stock}" readonly value="${p_data.Opening_Stock || ''}" class="form-control readonly-text-field smt_numeric_only opening-stock-details-field" tab-index="${j}">`,
    //                 `<select name="uom_list[${j}]" id="uom_list_${j}" class="form-control import_drop_down_sel"  tab-index="${j}">${generateOptions(uom_list, p_data.uom)}</select>`,
    //                 `<input name="ps_stock_price_[${j}]" id="ps_stock_price_${j}" type="text" title="${p_data.Stock_Price}" readonly value="${truncate(p_data.Stock_Price, 20)}" class="form-control readonly-text-field smt_numeric_only stock-price-details-field" tab-index="${j}">`,
    //                 `<input name="ps_brand_[${j}]" id="ps_brand_${j}" type="text" title="${p_data.Brand}" readonly value="${truncate(p_data.Brand, 100)}" class="form-control readonly-text-field brand-details-field">`,
    //                 `<input name="ps_buyer_product_name_[${j}]" id="ps_buyer_product_name_${j}" type="text" title="${p_data.Our_Product_Name}" readonly value="${truncate(p_data.Our_Product_Name, 100)}" class="form-control readonly-text-field brand-details-field" tab-index="${j}">`,
    //                 `<input name="ps_Inventory_Grouping_[${j}]" id="ps_Inventory_Grouping_${j}" type="text" title="${p_data.Inventory_Grouping}" readonly value="${truncate(p_data.Inventory_Grouping, 100)}" class="form-control readonly-text-field inventory-grouping-details-field" tab-index="${j}">`,
    //                 `<select name="invt_list[${j}]" id="invt_list_${j}" class="form-control">${generateOptions(invt_type_list, p_data.Inventory_Type)}</select>`,
    //                 `<input name="ps_Set_Min_Qty_for_Indent_[${j}]" id="ps_Set_Min_Qty_for_Indent_${j}" type="text" title="${p_data.Set_Min_Qty_for_Indent}" readonly value="${truncate(p_data.Set_Min_Qty_for_Indent, 10)}" class="form-control readonly-text-field smt_numeric_only Set_Min_Qty_for_Indent" tab-index="${j}">`
    //             ];
    ...
    //                 cnts++;
    //             } else {
    //                 console.warn(`Skipped row ${i + 1}: incorrect column count`, row);
    //             }
    //         }
    //     });
    //     if (data1.length > 0) {
    ...
    //                 data: data1,
    //                 columns: [
    //                     { title: "Sr. No", orderable: true },
    //                     { title: "Message", orderable: false },
    //                     { title: "Action", orderable: false },
    //                     { title: "Product Name", orderable: false },
    //                     { title: "Product Specification", orderable: false },
    //                     { title: "Product Size", orderable: false },
    //                     { title: "Opening Stock", orderable: false },
    //                     { title: "Product UOM", orderable: false },
    //                     { title: "Stock Price", orderable: false },
    //                     { title: "Brand", orderable: false },
    //                     { title: "Our Product Name", orderable: false },
    //                     { title: "Inventory Grouping", orderable: false },
    //                     { title: "Inventory Type", orderable: false },
    //                     { title: "Set Min Qty for Indent", orderable: false }
    //                 ],
    //                 scrollCollapse: false,//age true chilo
    //                 scroller: true,// age false
    //                 scrollY: 500,
    //                 searching: false,
    //                 autoWidth: false,
    //                 // pageLength: 25,
    ...
    //                         const rowTop = this.getBoundingClientRect().top;
    //                         const rowBottom = this.getBoundingClientRect().bottom;
    //                         const container = $('#example_wrapper .dataTables_scrollBody')[0].getBoundingClientRect();

    //                         if (rowBottom > container.top && rowTop < container.bottom) {
    //                             if (start === -1) start = index + 1; // +1 for 1-based index
    //                             end = index + 1;
    //                         }
    //                     });
