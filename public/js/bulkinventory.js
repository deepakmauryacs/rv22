$(document).ready(function () {
    ...
    function showimportdata(responsedata, uom_list, invt_type_list, callback) {
        ...
        responsedata.forEach((p_data, i) => {
            ...
            row.querySelector(".action-cell").innerHTML = `<button class="border-0 rounded bg-white remove-rfq-record btn-rfq" data-srno="${p_data.srno}"><i class="bi bi-trash3 text-danger remove-bulk-rfq-btn"></i></button>`;
            ...
            productNameDiv.innerHTML = `
                ...
                <div class="bulk-rfq-product-row">
                    <input type="text" title="${p_data.Product_Name}" readonly name="product_name[${j}]" id="product_name_${j}" tab-index="${j}" class="form-control bg-white readonly-text-field bulk-product-field bulk_product_fields" value="${p_data.Product_Name}" autocomplete="off">
            ...
            invtSelect.className = "form-select";
        ...
        $(".invalid-product-section").removeClass('d-none').css('display', 'inline');
        ...
        $('#example_wrapper .dataTables_scrollBody').off('scroll').on('scroll', throttledUpdate);
